...
def reshape_to_yolo_size(img):
    ...
    if min_edge < min_pixel:
        input_width *= float(min_pixel) / float(min_edge)
        input_height *= float(min_pixel) / float(min_edge)
...
def load_label_file(label_file):
    ...
    if os.path.isfile(label_file):
        ...
        print "%d Class found." %(len(labels))
        return labels
    else:
        print 'LABEL_FILE LOAD FAILED: %s' %(label_file)
        exit(1)


def load_train_list(traintxt):
    # TRAIN_FILEの存在チェック
    print 'Loading train file ...'
    if os.path.isfile(traintxt):
        with open(traintxt, "r") as f:
            trainfiles = f.read().strip().split("\n")
        return trainfiles
    else:
        print 'TRAIN_FILE LOAD FAILED: %s' %(traintxt)
        exit(1)


def load_gtfile(trainfiles, labels):
    """

    :param trainfiles: 学習ファイルのリスト
    :param labels: クラスラベルのリスト
    :return:
    """

    print "loading Teacher Files"
    # Ground Truthの読み込み
    t = []
    for path in trainfiles:
        # imageファイルからlabelファイルのpath変換
        gt_file = os.path.realpath(os.path.join(path, '../../labels/' + path.split(".")[0].split("/")[-1] + ".txt"))
        # pathにファイルがあるかチェック
        if os.path.isfile(gt_file):
            bbox = [0] * 4
            gt = []
            with open(gt_file, "r") as f:
                reader = csv.reader(f,delimiter=' ')# delimiter with space
                for row in reader:
                    # print int(row[0])
                    # print tuple(map(float,row[1:]))
                    class_id = int(row[0])
                    bbox[0], bbox[1], bbox[2], bbox[3] = tuple(map(float,row[1:]))
            ...
            t.append([path,gt])
    print "total %d label files loaded." %(len(t))
    return t
