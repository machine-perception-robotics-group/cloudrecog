...
def random_overlay_image(src_image, overlay_image, minimum_crop):
    ...
    y = int(np.random.randint(abs(src_h - scale_item_h)) - shift_item_h)
    x = int(np.random.randint(abs(src_w - scale_item_w)) - shift_item_w)
...
def image_transform(src):
    ...
    choices = np.random.choice(list,np.random.randint(1,len(list)-1))
    # print choices
    for select in choices:
        if select == 1:
            ...
            # 平滑化フィルタ
            square = np.random.randint(1,9)
            sp_img = blur_img = cv2.blur(src, (square, square))

        elif select == 5:
            # ガウシアンノイズ
            mean = 0
            sigma = np.random.randint(1,16)
            ...
            #ガンマ1
            sp_img = cv2.LUT(sp_img, LUT_G1)

        elif select == 7:
            #ガンマ2
            sp_img = cv2.LUT(sp_img, LUT_G2)
    ...
    def generate_random_animation(self, loop, bg_index, crop_width, crop_height, min_item_scale, max_item_scale):
        ...
        for i in range(loop):
            ...
            for j in range(animate_frames):
                ...
                frame = overlay(sampled_background, item, start_point[0] + int(w_distance * j / animate_frames),
                                start_point[1] + int(h_distance * j / animate_frames))
                frames.append(frame[:, :, :3])
        return frames

    def generate_samples(self, n_samples, n_items, crop_width, crop_height, min_item_scale, max_item_scale, rand_angle,
                         minimum_crop, delta_hue, delta_sat_scale, delta_val_scale):
        x = []
        t = []
...
class sample_generator:
    ...
    def generate_samples(self, inputsize):
        x = []
        t = []
        # バッチサイズ分読み取り
        for i in range(self.batch_size):
            # カウンタが最後までいったらリセット
            if self.current_counter + i >= len(self.dataset):
                self.current_counter = 0
                self.epoch += 1
                print "reset"
            dataset = self.dataset[self.current_counter+i]
        ...
        self.current_counter = self.current_counter + i
        return x,t
