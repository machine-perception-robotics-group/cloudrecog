...
class YOLOv2(Chain):
    ...
    def __init__(self, n_classes, n_boxes):
        ...
        self.l.append(self.layer_1)
        self.l.append(self.layer_2)
        self.l.append(self.layer_3)
        self.l.append(self.layer_4)
        self.l.append(self.layer_5)
        self.l.append(self.layer_6)
        self.l.append(self.layer_7)

        self.high_resolution_feature = None


    def layer_1(self, h):
        h = F.leaky_relu(self.bias1(self.bn1(self.conv1(h), finetune=self.finetune)), slope=0.1)
    ...
    def layer_5(self, h):
        ...
        self.high_resolution_feature = reorg(h) # 高解像度特徴量をreorgでサイズ落として保存しておく
        return h
    ...
    def layer_7(self, h):
        assert self.high_resolution_feature is not None
        ###### new layer
        ...
        h = F.concat((self.high_resolution_feature, h), axis=1) # output concatnation
        h = F.leaky_relu(self.bias21(self.bn21(self.conv21(h), finetune=self.finetune)), slope=0.1)
        h = self.bias22(self.conv22(h))
        return h
    def get_high_resolution_feature(self):

        return self.high_resolution_feature

    def set_high_resolution_feature(self, high_resolution_feature):
        self.high_resolution_feature = high_resolution_feature

    def __call__(self, h, mode, period):
        ##### common layer
        ...
        if mode == 0:
            ...
            for i in range(period,len(self.l)):
                h = self.l[i](h)
        return h
...
class YOLOv2Predictor(Chain):
    ...
    def predict_inputside(self, input_x, period):
        output = self.predictor(input_x, 0, period)
        return output
    def predict_outputside(self, input_x, period):
        output = self.predictor(input_x, 1, period)
        return output

    def calc_bbox(self,input_x):
        batch_size, _, grid_h, grid_w = input_x.shape
        x, y, w, h, conf, prob = F.split_axis(
            F.reshape(input_x, (batch_size, self.predictor.n_boxes, self.predictor.n_classes + 5, grid_h, grid_w)),
            (1, 2, 3, 4, 5), axis=2)
        x = F.sigmoid(x)  # xのactivation
        y = F.sigmoid(y)  # yのactivation
        conf = F.sigmoid(conf)  # confのactivation
        ...
        w_anchor = Variable(np.broadcast_to(
            np.reshape(np.array(self.anchors, dtype=np.float32)[:, 0], (self.predictor.n_boxes, 1, 1, 1)), w.shape))
        h_anchor = Variable(np.broadcast_to(
            np.reshape(np.array(self.anchors, dtype=np.float32)[:, 1], (self.predictor.n_boxes, 1, 1, 1)), h.shape))
        # x_shift.to_gpu(), y_shift.to_gpu(), w_anchor.to_gpu(), h_anchor.to_gpu()
        ...
        return box_x.data, box_y.data, box_w.data, box_h.data, conf.data, prob.data
    # def predict(self, input_x, mode, period):
    #     output = self.predictor(input_x, mode, period)
    #     print dir(self.predictor)
    #     high_resolution_feature = self.predictor.high_resolution_feature
    #     print high_resolution_feature
    #     if mode == 1:
    ...
    #     elif mode == 0:
    #         return output, high_resolution_feature
